...
(function($) {
  ...
  // Evita navegación normal y abre con Lity
  e.preventDefault();
  ...
  lity(href);
    ...
    /* Lity setup */
      // Inicializa el comparador cuando el contenido del modal está listo
      $(document).on('lity:ready', function () {
        var $modal = $('.lity-content');          // contenido actual del modal
        var el = $modal.find('.compare')[0];
        if (!el) return;                          // este modal no es el comparador

        // Evitar que Lity intercepte clic/drag dentro del comparador
        ...
        function setPosFromClientX(clientX){
          ...
          var p = (x / r.width) * 100;           // porcentaje a la IZQUIERDA (DESPUÉS)
          el.style.setProperty('--pos', p + '%');
        }
        function center(){ el.style.setProperty('--pos', '50%'); }

        // Centrar al abrir y al cargar imágenes
        center();
        el.querySelectorAll('img').forEach(function(img){
          if (!img.complete) img.addEventListener('load', center, { once:true });
        });
        var onResize = function(){ center(); };
        window.addEventListener('resize', onResize, { passive:true });

        // Arrastre con Pointer Events (desktop + móvil)
        var dragging = false;

        function start(e){
          dragging = true;
          el.setPointerCapture && el.setPointerCapture(e.pointerId || 1);
          setPosFromClientX(e.clientX ?? (e.touches && e.touches[0].clientX));
          e.preventDefault();
        }
        function move(e){
          if (!dragging) return;
        ...
        function end(){
          dragging = false;
          el.releasePointerCapture && el.releasePointerCapture(1);
        }
        ...
        // Limpieza al cerrar el modal (evita listeners duplicados)
